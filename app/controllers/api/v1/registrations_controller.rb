...
def create
  if params[:email].nil? 
    render :json=>{:code=> 500, :status=>"Failure", :message=>"Email can't be blank !"}
  else
    email = User.find_by_email(params[:email])
    if email.present?
     render :json=>{ :code=> 500, :status=>"Failure", :message=>"Email is already exist !" }
    else
      ...
      if user.save
        ...
        render :json=>{ :code=> 200, :status=>"Success", :message=>"Please check your email.", :user=>{email: user.email, authentication_token: user.authentication_token, created_at: user.created_at, updated_at: user.updated_at} }
      else
        render  :json=>{ :code=> 500, :status=>"Failure", :message=>"Your email id is wrong."}
      end
    end
  end  
end
