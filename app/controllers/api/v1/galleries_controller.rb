...
#   Parameters: none 
  ...
  def record 
    if params[:id].nil?
      render :json=>{:code=> 500, :status=>"Failure", :message=>"Id not exist in url ? Please append record id in url."}
    else
      @gallery = current_user.galleries.find(params[:id])
      if @gallery.present?
        render :json=>{:code=> 200, :status=>"Success", :message=>"Record found.", :data => {name: @gallery.try(:title), image_url: @gallery.try(:image).try(:url), }  }
      else
        render :json=>{:code=> 500, :status=>"Failure", :message=>"Record not exits."}
      end
    end
  end
  ...
  def create
    ...
    render :json=>{:code=> 500, :status=>"Failure", :message=>"Fields can't be blank !"}
      ...
      if (gallery.try(:title) != params[:title].downcase) && (gallery.try(:user).try(:id) != current_user.try(:id))
        ...
        if @gallery.save
        	render :json=>{ :code=>200, :status=>"Success", :message=>"Gallery Created.", :gallery => @gallery }
          else
        	render :json=>{ :code=>500,  :status=>"Failure", :message=>"Something went wrong" }
        end
      else
        render :json=>{ :code=>500,  :status=>"Failure", :message=>"Record exist." }
      end
    end
  end
